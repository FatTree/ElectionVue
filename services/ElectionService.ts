import { formatProfile, formatTheme, formaterParties, groupByParty } from '~/formatters/electionFormatter';
import type { ProfileListModel, ThemeListModel, PartyListModel, PartyModel } from '~/models/ElectionModel';
...
const GET_PARTY_LIST_URL = 'https://db.cec.gov.tw/static/elections/data/tickets/ELC/P0/00/';
...
export const getPartyList = async (
  ...
  try {
    ...
    const data = formaterParties(res[code]);
    return res[code].reduce(groupByParty, []);
  } catch (err) {
    console.error(err);
  }
  return [];
